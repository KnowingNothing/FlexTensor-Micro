...
base_res = subprocess.Popen(
    "python conv2d_baseline.py --type pytorch --target cuda --shapes res --number 100", shell=True, stdout=subprocess.PIPE)
...
plt.bar(x, base_bars, width=0.4, label="pytorch", fc="g")
x = [i + 0.4 for i in x]
plt.bar(x, flex_bars, width=0.4, label="flextensor", tick_label=names, fc="r")
plt.legend()
plt.show()
plt.savefig("resnet_cuda_performance_compare_tmp.png")