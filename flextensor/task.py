...
def conv2d_nchwc_layout(N, C, H, W, K, k=3, st=1, pad=0, dilation=1, group=1, vlen1=8, vlen2=8):
    ...
    inputs = tvm.placeholder(
        [N, C // vlen1 // group, H, W, vlen1], dtype="float32")
    ...
    if use_bias:
        ...
        bias = None
    output = conv2d_nchwc(inputs, weight, bias, stride=st,
...
def conv_transpose2d(N, C, H, W, K, kernel_size, stride=1, padding=0, output_padding=0, dilation=1, groups=1):
    ...
    Output = conv_transpose2d_nchw(Inputs, W, stride=stride, padding=padding,
...
def maxunpooling1d(N, C, L, kernel_size, stride=1, padding=0):
    ...
    Output = op_maxunpool1d(Input, Indices, kernel_size,
...
def maxunpooling2d(N, C, H, W, kernel_size, stride=1, padding=0):
    ...
    Output = op_maxunpool2d(Input, Indices, kernel_size,
                            stride=stride, padding=padding)
    return [Output.op], [Input, Indices, Output]
...
def pixelcnn(N, H, W, C, OutC, kernel_height, kernel_width, mask_type, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    Mask, Output = op_pixel_cnn(
        Input, Kernel, mask_type, bias=bias, dilation=dilation, stride=stride, padding=padding)
...
def gatedpixelcnn(N, H, W, C, OutC, kernel_size, ClassVector=None, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    GateV, Output = op_gated_pixel_cnn(Input, KernelV, KernelV2H, KernelH, KernelHOut,
                                       ClassVector, bias=bias, dilation=dilation, stride=stride, padding=padding)
...
for shape in gemm_shapes:
    N, K, M = shape
    for j in range(4):
        register_task(Task("gemm", "gemm", gemm,
                      (N, K, M, "int8"), "micro", j))
        register_task(Task("gemm", "gemm", gemm_uint8_int8,
                      (M, N, K, "int32"), "llvm -mcpu=skylake-avx512", j))
        register_task(Task("gemm", "gemm", gemm_uint8_int8,
                      (M, N, K, "int32"), "llvm -mcpu=cascadelake", j))
        register_task(Task("gemm", "gemm", gemm,
                      (N, K, M, "float32"), "llvm", j))
        register_task(Task("gemm", "gemm", gemm,
                      (N, K, M, "float32"), "cuda", j))
...
for shape in maxunpooling1d_shape:
    for j in range(4):
        register_task(Task("maxunpooling1d", "maxunpooling1d",
                      maxunpooling1d, shape, "llvm", j))
        register_task(Task("maxunpooling1d", "maxunpooling1d",
                      maxunpooling1d, shape, "cuda", j))

for shape in maxunpooling2d_shape:
    for j in range(4):
        register_task(Task("maxunpooling2d", "maxunpooling2d",
                      maxunpooling2d, shape, "llvm", j))
        register_task(Task("maxunpooling2d", "maxunpooling2d",
                      maxunpooling2d, shape, "cuda", j))
...
for shape in gated_pixelcnn_shape:
    for j in range(4):
        register_task(Task("gatedpixelcnn", "gatedpixelcnn",
                      gatedpixelcnn, shape, "llvm", j))
        register_task(Task("gatedpixelcnn", "gatedpixelcnn",
                      gatedpixelcnn, shape, "cuda", j))

for shape in shift_conv2d_shape:
    for j in range(4):
        register_task(Task("shift_conv2d", "shift_conv2d",
                      shiftconv2d, shape, "llvm", j))
        register_task(Task("shift_conv2d", "shift_conv2d",
                      shiftconv2d, shape, "cuda", j))
